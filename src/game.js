...
import { updatePlacement } from './placement.js';
import { updateHints } from './ui/hints.js';
import { menuItemsForTile, craftEntries, craft } from './interactions.js';
import { createStartInventory, swap } from './inventory.js';
...
export function createGame({ canvas, menuEl, paletteEl, inventoryEl, status }) {
  ...
  const state = {
    input, world: null, player: null, zombies: [], drops: [], arrows: [], inventory: createStartInventory(),
    swing: null, bowCool: 0, toolCool: 0, target: null, ghost: null, aim: 0, hint: null,
    held: 0, hotbarAnim: 0, camera: { x: 0, y: 0 }, vision: { visible: new Set(), exposed: true },
  ...
  function restart() {
    ...
    state.drops = []; state.arrows = []; state.inventory = createStartInventory();
    state.swing = null; state.bowCool = 0; state.toolCool = 0; state.target = null; state.ghost = null; state.hint = null;
    state.held = 0; state.attention = null; state.kills = 0; state.time = 0; state.paused = false;
  ...
  function update(dt) {
    ...
    if (palette.consumeJustClosed()) a.use = false;         // the click that dismissed the palette isn't a swing
    if (a.inventory) { if (invPanel.isOpen) invPanel.close(); else { palette.close(); invPanel.open(state.inventory); } }
    // Escape closes the topmost thing; with nothing open it toggles pause. There is no "build
    // mode" to leave — holding a placeable item IS the mode, and picking another slot leaves it.
    if (a.escape) {
    ...
    updateCamera();
    // Each of these no-ops unless the selected hotbar slot holds the kind of item it cares
    // about, so there is nothing to gate on "mode" — the held item IS the mode.
