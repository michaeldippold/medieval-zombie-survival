...
export function updateCombat(state, dt) {
  ...
  if (input.actions.slot && SLOTS[input.actions.slot - 1] && SLOTS[input.actions.slot - 1] !== state.held) {
    state.held = SLOTS[input.actions.slot - 1]; state.hotbarAnim = UI.HOTBAR_POP;
  }
  ...
  for (let i = state.arrows.length - 1; i >= 0; i--) {
    ...
    if (!a.arrived) {
      ...
      a.x = nx; a.y = ny;
      if (k >= 1) { a.arrived = true; if (a.target) { hurtZombie(state, a.target, BOW.DMG, a.ox); state.arrows.splice(i, 1); } }
    } else { a.stick += dt; if (a.stick > BOW.ARROW_STICK) state.arrows.splice(i, 1); }
  }
}
